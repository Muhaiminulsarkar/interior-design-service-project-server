...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.pssllgc.mongodb.net/?retryWrites=true&w=majority`;
console.log(uri);
...
async function run() {
  try {
    ...
    const servicesCollection = client.db("interiorhub").collection("services");
    const reviewsCollection = client.db("interiorhub").collection("reviews");
...
app.listen(port, () => {
  console.log(`interior design server ${port}`);
});
